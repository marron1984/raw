...
大阪府大阪市中央区南久宝寺町四丁目5番17号 アップウェル船場 2F
 訪問介護事業所 いいかいご
 TEL：06-4400-9333 FAX：06-4400-9333
法人設立年月日
2000年8月16日
...
■ 身体介護の利用料（利用者負担額は1割の場合）
| 時間帯＼サービス提供時間数 | 20分未満（単位数163） | 20分以上30分未満（単位数244） | 30分以上1時間未満（単位数387） | 1時間以上30分を増すごと（単位数567+82） |
| 昼間（午前8時～午後6時） | 1,812円（負担額182円） | 2,713円（負担額272円） | 4,303円（負担額431円） | 6,305円に911円を加算（負担額631円に92円を加算） |
| 早朝（午前6時～8時）・夜間（午後6時～10時） | 2,268円（負担額227円） | 3,391円（負担額340円） | 5,382円（負担額539円） | 7,884円に1,145円を加算（負担額789円に115円を加算） |
| 深夜（午後10時～午前6時） | 2,724円（負担額273円） | 4,069円（負担額407円） | 6,460円（負担額646円） | 9,463円に1,367円を加算（負担額947円に137円を加算） |
...
■ 生活援助の利用料（利用者負担額は1割の場合）
| 時間帯＼サービス提供時間数 | 20分以上45分未満（単位数179） | 45分以上（単位数220） |
| 昼間 | 1,990円（負担額199円） | 2,446円（負担額245円） |
| 早朝・夜間 | 2,490円（負担額249円） | 3,058円（負担額306円） |
| 深夜 | 2,991円（負担額300円） | 3,669円（負担額367円） |

■ ★１ 通院等乗降介助の利用料（片道の料金・利用者負担額は1割の場合）
| 時間帯 | 昼間（単位数97） | 早朝・夜間（単位数121） | 深夜（単位数146） |
| 利用料（負担額） | 1,078円（負担額108円） | 1,345円（負担額135円） | 1,623円（負担額163円） |

■ ★２ 加算（利用者負担額は1割の場合・要介護度による区分なし）
| 加算の種類 | 利用料 | 利用者負担額 | 算定回数等 |
| 特定事業所加算 | （Ⅰ）所定単位数の20/100（Ⅱ）10/100（Ⅲ）10/100（Ⅳ）3/100（Ⅴ）3/100 | 左記の1割 | 1回当たり |
| 緊急時訪問介護加算 | 1,112円 | 112円 | 1回の要請に対して1回 |
| 初回加算 | 2,224円 | 223円 | 初回のみ |
| 中山間地域等における小規模事業所加算 | 所定単位数の10/100 | 左記の1割 | 1回当たり |
| 中山間地域等に居住する者へのサービス提供加算 | 所定単位数の5/100 | 左記の1割 | 1回当たり |
| 生活機能向上連携加算 | （Ⅰ）1,112円（Ⅱ）2,224円 | （Ⅰ）112円（Ⅱ）223円 | 1月当たり |
| 訪問介護認知症専門ケア加算 | （Ⅰ）33円（Ⅱ）44円 | （Ⅰ）4円（Ⅱ）5円 | 1日につき |
| 口腔連携強化加算 | 556円 | 56円 | 1月に1回限り |
| 介護職員等処遇改善加算 | 所定単位数の（Ⅰ）245/1000（Ⅱ）224/1000（Ⅲ）182/1000（Ⅳ）145/1000（Ⅴ1）221/1000（Ⅴ2）208/1000（Ⅴ3）200/1000（Ⅴ4）187/1000（Ⅴ5）184/1000（Ⅴ6）163/1000（Ⅴ7）163/1000（Ⅴ8）158/1000（Ⅴ9）142/1000（Ⅴ10）139/1000（Ⅴ11）121/1000（Ⅴ12）118/1000（Ⅴ13）100/1000（Ⅴ14）76/1000 | 左記の1割 | 基本サービス費に各種加算減算を加えた総単位数（所定単位数）。※（Ⅴ）は令和7年3月31日まで算定可 |
...
訪問介護事業所 いいかいご
訪問介護事業部
所在地 大阪市天王寺区味原町13-18 オーセント味原302号室
電話番号・ﾌｧｯｸｽ番号 06-4400-9333
受付時間 午前9時〜午後6時
